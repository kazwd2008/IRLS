...
#       Ver. 3.2  2013/04/25  切片のない回帰用のTirls.avexと        #
#                               Tirls.madxを作成  					#
...
###################################################################
#	Tirls.avexとTirls.madxは、切片のない回帰を行う。
###################################################################
...
Tirls.ave <- function(y1, x1, rt=rep(1, length(y1)), c1=8, dat="", rp.max=150, cg.rt=0.01) {
  ...
  R0 <- lm(y1~x1, weights=rt)           # 初期値は普通のOLS
  
  Tk2 <- R0
  rp1 <- 1								# ループ回数
  s0 <- 0			# 最低１回はループするように差が0.01より大きくなるよう値をセット
  s1 <- s1.cg[rp1] <- mean(abs(Tk2$residuals-mean(Tk2$residuals)))       # 平均絶対残差(MAD)
  ...
  for (i in 2:rp.max) {                             # 2011.03.07
      ...
      if (abs(1-s1/s0) < cg.rt) break                # 2012.12.19
        ...
        rp1 <- rp1 + 1			# ループカウンタ
        s1 <- s1.cg[rp1] <- mean(abs(Tk2$residuals-mean(Tk2$residuals)))	# 平均絶対残差
        u1 <-Tk2$residuals/(c1*s1)
        w1 <- (1-u1**2)**2
        w1[which(abs(u1)>=1)] <- 0
    }
...
Tirls.avex <- function(y1, x1, rt=rep(1, length(y1)), c1=8, dat="", rp.max=150, cg.rt=0.01) {
  ...
  if (dat!="") attach(get(dat))	        # データフレーム指定があるときのみ
  s1.cg <- rep(0, rp.max)               # 2012.12.18 s1の変遷を保存
  R0 <- lm(y1~x1-1, weights=rt)           # 初期値は普通のOLS
  
  Tk2 <- R0
  rp1 <- 1								# ループ回数
  s0 <- 0			# 最低１回はループするように差が0.01より大きくなるよう値をセット
  s1 <- s1.cg[rp1] <- mean(abs(Tk2$residuals-mean(Tk2$residuals)))   # 平均絶対残差(AAD)
